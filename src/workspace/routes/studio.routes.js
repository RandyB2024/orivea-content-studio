...
router.get("/posts/:id/detail",(req,res)=>{const post=db.prepare(`SELECT p.*,c.title content_title,c.category,c.source_type,c.product_reference,c.usage_permission,c.campaign_id,cp.name campaign_name,cp.start_date campaign_start,cp.end_date campaign_end FROM studio_posts p JOIN content_items c ON c.id=p.content_item_id LEFT JOIN campaigns cp ON cp.id=c.campaign_id WHERE p.id=?`).get(req.params.id);if(!post)return res.status(404).json({error:"Post niet gevonden."});post.platforms=db.prepare("SELECT * FROM post_platforms WHERE post_id=? ORDER BY id").all(post.id);post.media=db.prepare("SELECT m.*,pm.position FROM post_media pm JOIN media_assets m ON m.id=pm.media_asset_id WHERE pm.post_id=? ORDER BY pm.position").all(post.id);if(!post.media.length)post.media=db.prepare("SELECT m.*,0 position FROM media_assets m WHERE content_item_id=? AND disabled=0 ORDER BY id LIMIT 1").all(post.content_item_id);post.sources=db.prepare("SELECT DISTINCT d.id,d.title,d.source_type,s.reason FROM post_knowledge_sources s JOIN knowledge_documents d ON d.id=s.document_id WHERE s.post_id=?").all(post.id);post.accounts=Object.fromEntries(db.prepare("SELECT platform,status FROM social_accounts").all().map(row=>[row.platform,row.status]));post.versions=db.prepare("SELECT * FROM post_versions WHERE post_id=? ORDER BY id DESC LIMIT 10").all(post.id).map(row=>({...row,platform_payload:JSON.parse(row.platform_payload)}));post.checks={product_match:Boolean(post.product_reference),campaign_valid:!post.campaign_end||new Date(post.campaign_end+"T23:59:59")>=new Date(),rights_ok:["own_content","approved","shared_by_glantier"].includes(post.usage_permission),media_relevant:Boolean(post.media.length),cta_valid:!post.target_url||/^https:\/\/(www\.)?orivea\.nl/i.test(post.target_url),no_duplicate:(post.quality_score||0)>=80,no_claim_flags:(post.quality_score||0)>=75};res.json(post);});
router.put("/posts/:id",(req,res)=>{const selected=parsePlatforms(req.body.platforms);if(!selected.length)return res.status(400).json({error:"Kies minimaal één platform."});db.transaction(()=>{db.prepare("UPDATE studio_posts SET title=?,cta=?,target_url=?,generated_url=?,scheduled_at=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(clean(req.body.title,180),clean(req.body.cta,120),clean(req.body.target_url,500),clean(req.body.generated_url,700),req.body.scheduled_at||null,req.params.id);for(const platform of selected)db.prepare("INSERT INTO post_platforms(post_id,platform,caption,hashtags,status) VALUES(?,?,?,?, 'draft') ON CONFLICT(post_id,platform) DO UPDATE SET caption=excluded.caption,hashtags=excluded.hashtags").run(req.params.id,platform,clean(req.body[`caption_${platform}`]),clean(req.body[`hashtags_${platform}`],500));db.prepare(`DELETE FROM post_platforms WHERE post_id=? AND platform NOT IN (${selected.map(()=>"?").join(",")})`).run(req.params.id,...selected);if(Number(req.body.media_id)){db.prepare("DELETE FROM post_media WHERE post_id=?").run(req.params.id);db.prepare("INSERT INTO post_media(post_id,media_asset_id,position) VALUES(?,?,0)").run(req.params.id,Number(req.body.media_id));}})();res.json({ok:true});});
router.get("/posts/:id/media-alternatives",(req,res)=>{const post=db.prepare("SELECT c.* FROM studio_posts p JOIN content_items c ON c.id=p.content_item_id WHERE p.id=?").get(req.params.id);if(!post)return res.status(404).json({error:"Post niet gevonden."});res.json(db.prepare(`SELECT m.*,c.title,((CASE WHEN c.product_reference=? THEN 40 ELSE 0 END)+(CASE WHEN c.campaign_id=? THEN 35 ELSE 0 END)+(CASE WHEN c.category=? THEN 15 ELSE 0 END)-c.times_used*5) score FROM media_assets m JOIN content_items c ON c.id=m.content_item_id WHERE m.disabled=0 AND c.usage_permission IN ('own_content','approved','shared_by_glantier') AND (c.product_reference=? OR c.campaign_id=? OR c.category=?) ORDER BY score DESC LIMIT 30`).all(post.product_reference,post.campaign_id,post.category,post.product_reference,post.campaign_id,post.category));});
router.post("/posts/:id/approve",(req,res)=>{const result=db.prepare("UPDATE studio_posts SET status='ready',approved_at=CURRENT_TIMESTAMP,approved_by=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(req.session.user.username,req.params.id);db.prepare("UPDATE post_platforms SET status='ready' WHERE post_id=?").run(req.params.id);res.json({ok:Boolean(result.changes),status:"ready"});});
router.post("/posts/:id/reject",(req,res)=>{db.prepare("UPDATE studio_posts SET status='rejected',updated_at=CURRENT_TIMESTAMP WHERE id=?").run(req.params.id);db.prepare("UPDATE post_platforms SET status='rejected' WHERE post_id=?").run(req.params.id);res.json({ok:true,status:"rejected"});});
