...
function importOrder(data, messageId) {
  ...
  const paymentStatus = verified ? "verification_required" : "verification_required";
  if (existing) {
  ...
  const result = db.prepare(`INSERT INTO orders (order_number,customer_name,customer_email,customer_phone,customer_address,order_date,order_items,subtotal,shipping_cost,total,paypal_order_id,paypal_transaction_id,payment_status,payment_method,status,newsletter_opt_in,source,mail_message_id,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'PayPal','Nieuw',?,'emailjs_email',?,?)`).run(data.order_number,data.customer_name || "",data.customer_email,data.customer_phone || "",data.customer_address || "",data.created_at || new Date().toISOString(),typeof data.items === "string" ? data.items : JSON.stringify(data.items || []),money(data.subtotal),money(data.shipping),money(data.total),data.paypal_order_id || "",data.capture_id || data.paypal_transaction_id || "",paymentStatus,bool(data.newsletter) ? 1 : 0,messageId,verified ? "PayPal-identificatie aanwezig; betaling nog server-side verifiëren." : "Betaling niet geverifieerd vanuit e-mail.");
